...
{% macro form_field(field, form_type = "basic", horizontal_columns = field.columns, button_map = {}) %}

    {# this is a workaround hack for the more straightforward-code of just passing required=required parameter. older versions of wtforms do not have the necessary fix for required=False attributes, but will also not set the required flag in the first
    place. we skirt the issue using the code below #}
    ...
    {%- elif field.type == 'SubmitField' -%}
        ...
        {% call _hz_form_wrap(horizontal_columns, form_type, True, required = required) %}
        {{field(class='btn btn-%s' % button_map.get(field.name, 'inverse'),
            **field_kwargs)}}
        {% endcall %}
...
4.2. Same-document References
 ...
 -#}
        {#- if any file fields are inside the form and enctype is automatic, adjust if file fields are found. could really use the equalto test of jinja2 here, but latter is not available until 2.8 warning: the code below is guaranteed to make you cry =( #}
        {%- set _enctype = [] %}
        {%- if enctype is none -%}
            {%- for field in form %}
                {%- if field.type == 'FileField' %}
